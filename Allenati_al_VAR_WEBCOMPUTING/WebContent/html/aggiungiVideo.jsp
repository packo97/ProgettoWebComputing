...
<div id="jumboAggiungi" class="container">

  			<h1><span class="badge">Aggiungi video</span></h1>	
		  		...
		  		<form id="formContent">
					...
					<div class="fadeIn second" id="categoria" >
						<select id="cat" name="categoria" required>
							...
							<option value="GIALLO">AMMONIZIONE</option>
					...
					<div class="fadeIn second" id="grado di difficoltà" >
						<select id="sel" name="grado di difficoltà" required>
							<option value="" disabled selected>GRADO DI DIFFICOLTA'</option>
		    ...
		    <div id="anteprimaVideo" class="column right"></div>
		</div> 
</div>
				...
				$.ajax({
					...
					success: function(info){
						$("document").ready(function(){
								...
								{
									window.location.href="gestoreVideo?aggiungiVideo=true&&link="+url+"&&nome="+nome+"&&desc="+descrizione+"&&diff="+valueDifficolta+"&&cat="+valueCategoria+"&&opC="+opzioneUno+"&&opE="+opzioneDue+"&&squadraA="+squadraA+"&&squadraB="+squadraB+"&&durata="+durata;
								}
								else
								{
									var $temp= $("form");
